...
const ProductPage = ({ product, layoutInfo, catalogpage }) => {
  ...
  return (
    ...
    <div className="m-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4 mt-8">
      ...
      <div className="rounded-t-lg pt-2 pb-2 m-auto h-40 w-40">
        <CustomImage image={product?.attributes.Image} />
      </div>
      <div className="w-full p-5 flex flex-col justify-between">
        <div>
          <h4 className="mt-1 font-semibold text-lg leading-tight truncate text-gray-700">
            {product.title} - ${product?.attributes.Price}
          </h4>
          <div className="mt-1 text-gray-600">{product?.attributes.ShortDescription}</div>
        </div>
